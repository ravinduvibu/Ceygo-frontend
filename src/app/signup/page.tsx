...
export default function SignUp() {
  ...
  return (
    <div className="flex min-h-screen w-full flex-col lg:flex-row bg-slate-50 relative">
      {/* Error Toast */}
      <div className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 ease-out transform ${error ? "translate-y-0 opacity-100" : "-translate-y-12 opacity-0 pointer-events-none"}`}>
      ...
      <div className="relative hidden w-full lg:flex lg:w-1/2 flex-col justify-between p-12 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <Image src="/images/sri-lanka-landscape.png" alt="Sri Lanka beautiful landscape" fill sizes="50vw" className="object-cover scale-105" priority />
        ...
        <div className="w-full max-w-md space-y-6 relative z-10">
          <div className="flex lg:hidden flex-col items-center space-y-4 mb-2 mt-4">
            <div className="relative h-16 w-40 p-2 drop-shadow-sm">
              <Image src="/images/logo_transparent.png" alt="Ceygo Logo" fill sizes="160px" className="object-contain p-1 dark:invert" priority />
            </div>
          </div>

          <div className="space-y-2 text-center lg:text-left">
            <h2 className="text-3xl font-bold tracking-tight text-[#0f172a]">Create an account</h2>
          ...
          <div className="glass-panel rounded-2xl p-6 sm:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
            <div className="mb-6">
              <RoleToggle roles={["Traveler", "Partner"]} defaultRole={role} onRoleChange={setRole} />
            </div>
            ...
            <form className="space-y-4" noValidate onSubmit={handleSubmit}>
                ...
                {password && (
                  <div className="mt-2 text-xs font-semibold text-slate-500 flex items-center space-x-2">
                    ...
                    <div className="flex-1 flex gap-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                      {[1, 2, 3, 4].map((n) => (
                        <div key={n} className={`h-full transition-all duration-300 ${pwStrength >= n ? pwStrengthColor : "bg-transparent"}`} style={{ width: "25%" }} />
                      ))}
                    </div>
                  </div>
                )}
              ...
              <div className="pt-2">
                <button type="submit" disabled={loading}
                  className="w-full rounded-xl bg-primary px-4 py-3.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-primary-hover hover:shadow-md hover:shadow-primary/20 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
